...
description: "Part 2 of building a Chip-8 emulator in Rust: initializing memory with font sprites and ROM loading, implementing the emulation loop at 600Hz, and managing delay and sound timers at 60Hz."
tags:
    - rust
    - chip-8
    - emulator
...
approximate speed of 600 instructions per second (600 Hz).
...
Since we have to run 600 CPU instructions every second. If we execute 10
instructions in every iteration, we can achieve approximately 10x60=600
...
In the next part, [Build a Chip-8 Emulator in Rust - Part 3 - Instruction
Decoding](./chip-8-emulator-in-rust-part-three), we'll decode instructions into
nibbles and wire up the first opcodes — register loads, jumps, and skips. After
that, `run_cycle` will actually do something.
